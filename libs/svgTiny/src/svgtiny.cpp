...
static svgtiny_code svgtiny_parse_image(Poco::XML::Element *image,struct svgtiny_parse_state state);
...
svgtiny_code svgtiny_parse(svgInfo &info,struct svgtiny_diagram *diagram, const char *buffer,int viewport_width, int viewport_height){
    ...
    info.xmlns_xlink = ofToString(svg->getAttribute("xmlns:xlink").c_str());
    ...
    if (child->nodeType() == Poco::XML::Element::ELEMENT_NODE) {
        ...
        if (strcmp(name, "svg") == 0){
           ...
           // ofLog()<<"defining currNode"<<endl;
            ...
            pDef.width = ofToString(child->getAttribute("width").c_str());
            pDef.height = ofToString(child->getAttribute("height").c_str());
            pDef.base64ImgData = ofToString(child->getAttribute("xlink:href").c_str());
            
            childnode->image = pDef;
            childnode->type = SVG_TAG_TYPE_IMAGE;
            node->children.push_back(childnode);
            state.currNode = childnode;
            
            code = svgtiny_parse_image(child, state);
        }
...
svgtiny_code svgtiny_parse_image(Poco::XML::Element *imgNode,
		struct svgtiny_parse_state state)
{
	float x, y, width, height;
	float px, py;
	...
	svgtiny_parse_position_attributes(imgNode, state,
			&x, &y, &width, &height);
...
/* 	state.ctm.f = py - state.origin_y; */
    ...
    Poco::XML::NodeIterator it(imgNode, Poco::XML::NodeFilter::SHOW_ELEMENT | Poco::XML::NodeFilter::SHOW_TEXT);
    Poco::XML::Node* pNode = it.nextNode();
    while (pNode) {
		...
		if (pNode->getNodeValue().compare("text") == 0) 
        {
            ...
            //currently just adding image node as text
            shape->text = strdup((const char *) pNode->getNodeValue().c_str());
